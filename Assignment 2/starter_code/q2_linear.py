...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        height, width, nchannels = list(self.env.observation_space.shape)
        ...
        self.s = tf.placeholder(tf.uint8, shape=(None, height, width, nchannels*self.config.state_history))
        self.a = tf.placeholder(tf.int32, shape=(None, ))
        self.r = tf.placeholder(tf.float32, shape=(None, ))
        self.sp = tf.placeholder(tf.uint8, shape=(None, height, width, nchannels*self.config.state_history))
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.compat.v1.variable_scope(scope) as vs:
            s = layers.flatten(state)
            out = layers.dense(s, num_actions, activation=None, reuse=reuse)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        v = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope)
        t = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
        self.update_target_op = tf.group([tf.assign(t[i], v[i]) for i in range(len(v))])
    ...
    def add_loss_op(self, q, target_q):
        ...
        to_do = 1 - tf.cast(self.done_mask, tf.float32)
        Q_target = self.r + to_do * self.config.gamma * tf.reduce_max(target_q, axis=1)
        Q_sample = tf.reduce_max(q*tf.one_hot(self.a, num_actions), axis=1)
        self.loss = tf.reduce_mean(tf.square(Q_target - Q_sample))
